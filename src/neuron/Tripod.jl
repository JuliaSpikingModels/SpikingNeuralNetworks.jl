...
EyalGluDend = SNNUtils.EyalGluDend
...
function integrate!(p::Tripod, param::AdExTripod, dt::Float32)
    ...
    # for soma
    @inbounds @fastmath for (n, rec) in enumerate(soma_receptors)
        @unpack gsyn, α, E_rev, τr⁻, τd⁻ = rec
        @fastmath for i = 1:N
...
function update_tripod!(p::Tripod, Δv::Vector{Float32}, i::Int64, param::AdExTripod, dt::Float32)
    ...
    _c1 = -( (v_d1[i] + Δv[2] * dt) - (v_s[i] + Δv[1] * dt)) * d1[i].gax
    _c2 = -( (v_d2[i] + Δv[3] * dt) - (v_s[i] + Δv[1] * dt)) * d2[i].gax

    # compute synaptic currents
    _i1 = clamp(syn_current(v_d1[i] + Δv[2] * dt, g_d2[i, :], p.dend_syn), -500.f0,500.f0)
    _i2 = clamp(syn_current(v_d2[i] + Δv[3] * dt, g_d2[i, :], p.dend_syn), -500.f0,500.f0)
    _is = clamp(syn_current_soma(v_s[i] + Δv[1] * dt, g_s[i, :], p.soma_syn), -500.f0,500.f0)

    # update membrane potential
    Δv[1] = ΔvAdEx(v_s[i] + Δv[1] * dt, w_s[i], θ[i], _c1 + _c2, _is, param)
    Δv[2] = ΔvDend(v_d1[i] + Δv[2] * dt, -_c1, _i1, d1[i])
    Δv[3] = ΔvDend(v_d2[i] + Δv[3] * dt, -_c2, _i2, d2[i])
    end
...
@inline
@fastmath function ΔvDend(v::Float32, axial::Float32, synaptic::Float32, pm::Dendrite)::Float32
...
@inline
function NMDA_nonlinear(NMDA::ReceptorVoltage, v::Float32)::Float32

    return (1 + (NMDA.mg / NMDA.b) * exp32(NMDA.k * (v)))^-1 ##NMDA
end
@inline
function syn_current(v::Float32, g::Vector{Float32}, syn::Synapse)
...
@inline
function syn_current_soma(v::Float32, g::Vector{Float32}, syn::Synapse)
