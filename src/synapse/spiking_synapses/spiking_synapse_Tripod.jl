...
@snn_kw struct SynapseNormalization{
    ...
    param::NormParam = MultiplicativeNorm(0)
    t::VIT = [0, 1]
    W0::VFT = [0.0f0]
    W1::VFT = [0.0f0]
    μ::VFT = [0.0f0]
...
function SynapseNormalization(N::Int = 0, param::NormParam = MultiplicativeNorm())
    W0 = zeros(Float32, N)
    W1 = zeros(Float32, N)
    μ = zeros(Float32, N)
    SynapseNormalization(; @symdict(param, W0, W1, μ)...)
end
...
function forward!(c::SynapseTripod, param::SpikingSynapseParameter)
    ...
    @inbounds @simd for j = 1:(length(colptr)-1) # loop on presynaptic neurons
        if fireJ[j] # presynaptic fire
            for a in eachindex(αs)
